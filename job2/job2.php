...
<body>
    ...
    { 
        try{
            ...
            $job = $_POST["job"];
            $ql = $_POST["ql"];
            $regist = $pdo->prepare("INSERT INTO job(date,company,address,job,ql) VALUES(:date,:company,:address,:job,:ql)");
    ...
    <h2>データ入力</h2>
	<form id = "entry" action="job2.php" method="post" role="form" onSubmit="return check()">
	...
	<?php
	// データベースからデータを取得する
	$sql = "SELECT id,year,month,day,company,address,job,ql FROM job;";
	$stmt = $pdo->prepare($sql);
	$stmt -> execute();
	?>
	<table>
