###### Intune Mobileアプリ構成プロファイル情報取得スクリプト    　#####
###### Create Date   2021/8/26                                    #####
###### Create Author  T.HATTORI                                   #####
##出力ファイル
$result_file = "C:\work\20210826\result\Get-IntuneMobileAppConfigurationPolicy.txt"
#行数カウント
$count=0
...
## コマンド実行メソッド
function command {
    Get-IntuneMobileAppConfigurationPolicy | Format-List | Out-File $result_file 
    return_code ($?)
}
