######   Intune 管理デバイスマネージメントポリシー情報取得スクリプト##########
######   Create Date    2021/8/25                                   ##########
######   Create Author  T.HATTORI                                   ##########
##############################################################################

#出力ファイルの定義
$result_file = "C:\work\20210825\result\Get_IntuneManagedDeviceDeviceCompliancePolicyState.txt"
#メッセージの定義
$msg1 = "OK"
$msg2 = "NG"
$msg3 = "ファイルへの出力が成功しました"
$msg4 = "ファイルへの出力に失敗しました"
$msg5 = "異常終了です"
...
#デバイス名の配列データ
$DID = @(
        'd4ae3200-c61a-4f8d-b47d-3f6fab590ca5'
        )
...
function command {
    foreach ($line in $DID){
        Get-IntuneManagedDeviceDeviceCompliancePolicyState -managedDevice $line | Format-List | Out-File $result_File -Append
        return_code($?)
    }
} 
