######   Intune モバイルアプリ情報取得スクリプト########
######   Create Date    2021/8/25             ##########
######   Create Author  T.HATTORI             ##########
########################################################

#出力ファイルの定義
$result_file = "C:\work\20210826\result\Get-IntuneMobileApp1.txt"
#メッセージの定義
$msg1 = "OK"
$msg2 = "NG"
...
function command($flag) {
    Get-IntuneMobileApp | Format-List | Select-String $flag |  Out-File $result_File -Append
    return_code($?)
}
...
#メインメソッド処理
$APP=@( 'IE11','AcrobatReader','Hidemaru')
foreach ($line in $APP){
	command($line)
}
